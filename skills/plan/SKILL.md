...
3. IMMEDIATELY after writing plan.md, move the ticket to **In Planning**, using the method determined by the **Backend:** field in workflow.md:
   ...
   - **Jira backend:** via the Atlassian MCP `editJiraIssue` tool on the key in the ticket's `jira_issue` frontmatter — remove any `phase:*` label and add `phase:in-planning`. Then read the **Phase → Transition map** from `workflow.md`. If the row for `phase:in-planning` is not `skip`, call `getTransitionsForJiraIssue` on `{jira_issue}`, match the configured transition name case-insensitively against `transitions[].name` to get its `id`, then call `transitionJiraIssue` with that `id`. If the configured name is not currently available (workflow guard) or the row is `skip`, continue without erroring — the label swap is authoritative.
