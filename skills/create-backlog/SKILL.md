...
Before doing anything, read `memory.md` in the repo root if it exists, then resolve and read **`workflow.md`** per **`skills/conventions/01-plugin-root-and-templates.md`** and record the **Backend:** value. If it is still the unresolved `[CONFIGURE: github | jira]` placeholder, stop and tell the user to finish `/project-start` first. After triage, update `memory.md` if you recorded any new team-wide pattern worth persisting (per `CLAUDE.md` Agent rules if present).
